...
function performSearch() {
  ...
  function updateItemsPerPage() {
    ...
    if (itemsPerPage === 12) {
        ...
        if (currentPage.startsWith("me") || currentPage.startsWith("ban") || currentPage.startsWith("re") || currentPage.startsWith("sang") || currentPage.startsWith("ny")) {
            const baseName = currentPage.replace(/[1-3]$/, ''); // Xóa số 1, 2 hoặc 3 ở cuối
            window.location.href = `${baseName}4.html`;
        } else {
            console.error("File hiện tại không đúng định dạng (ví dụ: me1, ban2, re3).");
        }
